...
def test_validate_number_positive_integer():
    assert InputValidator.validate_number(324, config) == Decimal('324')

def test_validate_number_positive_decimal():
    assert InputValidator.validate_number(674.323, config) == Decimal('674.323').normalize()

def test_validate_number_positive_string_integer():
    assert InputValidator.validate_number("658", config) == Decimal('658')

def test_validate_number_positive_string_decimal():
    assert InputValidator.validate_number("972.777", config) == Decimal('972.777').normalize()
