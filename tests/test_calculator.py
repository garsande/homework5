...
def test_undo(calculator):
    operation = OperationFactory.create_operation('add')
    calculator.set_operation(operation)
    calculator.perform_operation(11, 3)
    calculator.undo()
    assert calculator.history == []
...
def test_redo(calculator):
    operation = OperationFactory.create_operation('add')
    calculator.set_operation(operation)
    calculator.perform_operation(6, 3)
    calculator.undo()
    calculator.redo()
...
def test_save_history(mock_to_csv, calculator):
    operation = OperationFactory.create_operation('add')
    calculator.set_operation(operation)
    calculator.perform_operation(97, 3)
...
def test_save_history_error(mock_to_csv, calculator):
    try:
        operation = OperationFactory.create_operation('add')
        calculator.set_operation(operation)
        calculator.perform_operation(47, 3)
        calculator.save_history()
    except OperationError as e:
        pytest.fail(f"Failed to save history: {e}")
