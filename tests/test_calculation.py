...
def test_addition():
    calc = Calculation(operation="Addition", operand1=Decimal("9"), operand2=Decimal("3"))
...
def test_multiplication():
    calc = Calculation(operation="Multiplication", operand1=Decimal("6"), operand2=Decimal("2"))
    assert calc.result == Decimal("12")


def test_division():
    calc = Calculation(operation="Division", operand1=Decimal("24"), operand2=Decimal("2"))
    assert calc.result == Decimal("12")
...
def test_power():
    calc = Calculation(operation="Power", operand1=Decimal("3"), operand2=Decimal("3"))
    assert calc.result == Decimal("27")


def test_negative_power():
    with pytest.raises(OperationError, match="Negative exponents are not supported"):
        Calculation(operation="Power", operand1=Decimal("5"), operand2=Decimal("-3"))


def test_root():
    calc = Calculation(operation="Root", operand1=Decimal("4"), operand2=Decimal("2"))
    assert calc.result == Decimal("2")


def test_invalid_root():
    with pytest.raises(OperationError, match="Cannot calculate root of negative number"):
        Calculation(operation="Root", operand1=Decimal("-10"), operand2=Decimal("2"))


def test_unknown_operation():
    with pytest.raises(OperationError, match="Unknown operation"):
        Calculation(operation="Unknown", operand1=Decimal("9"), operand2=Decimal("3"))
